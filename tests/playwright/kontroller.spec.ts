...
test("Karıştır kart sırasını değiştirir ama seçimi ve hakları korur", async ({ page }) => {
  ...
  await expect.poll(async () => (await tahtaSirasi(page)).join("|")).not.toBe(oncekiSira.join("|"));
...
test("dörtten fazla kelime seçilemez", async ({ page }) => {
  ...
  await expect(secimSayaci(page)).toHaveText(`${GAME_CONSTANTS.groupSize}/${GAME_CONSTANTS.groupSize} seçili`);
