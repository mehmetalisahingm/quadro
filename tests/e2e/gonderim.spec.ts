/**
 * Gönderim sonuçlarının tarayıcı regresyonları: çok yakın, tekrar, hızlı çift gönderim
 * ve dördüncü hatayla kaybetme.
 */
...
test("tam üç doğru kelime çok yakın sonucu verir ve bir hak götürür", async ({ page }) => {
  ...
  // Çok yakın sonuç hiçbir grubu çözmez: tahta olduğu gibi kalır.
...
test("aynı dörtlü farklı sırayla gönderilince hak eksilmez", async ({ page }) => {
  ...
  // Aynı dört kelime, ters sırayla: tekrar denetimi sıradan bağımsızdır.
  await temizle(page);
...
test("Grupla'ya hızlı çift basış tek gönderim olarak işlenir", async ({ page }) => {
  ...
  // Gerçek tarayıcıda iki tıklama arka arkaya: ikinci gönderim ya UI kilidine takılır
  // ya da motorun tekrar denetimine düşer; her iki durumda tek hak eksilir.
  await gonderDugmesi(page).dblclick();

  await kalanHakOlmali(page, GAME_CONSTANTS.maxMistakes - 1);

  // Tek hak eksilmesi yetmez: geçmişe de tek tahmin yazılmalı. Oyunu tamamlayıp
  // paylaşım ızgarasının satır sayısını sayıyoruz (1 başlık + 1 yanlış + 4 doğru).
  await temizle(page);
...
test("dördüncü yanlışta oyun biter ve kalan cevaplar açılır", async ({ page }) => {
  ...
  // Hiçbir grup bulunmadığı için dördü de "Cevap" olarak açıklanır.
...
test("kaybedilen oyunda tahta ve kontroller kaybolur", async ({ page }) => {
  ...
  // Tahta render edilmediği için bitmiş oyunda tıklanacak kart kalmaz.
